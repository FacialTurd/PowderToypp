	...
	legacy(new TPTScriptInterface(c, m))
{
	...
	const static struct luaL_Reg tptluaapi [] = {
		...
		{"add_dbg_trigger", trigger_func_struct::_add},
		{"call_dbg_trigger", trigger_func_struct::_call},
...
int LuaScriptInterface::interface_addNotification(lua_State * l)
{
	...
	public:
		...
		virtual void Action()
		{
			int idx = lua_gettop(ls0);
			lua_rawgeti(ls0, LUA_REGISTRYINDEX, ref0);
			lua_pcall(ls0, 0, 1, 0);
			bool cl = lua_toboolean(ls0, -1);
			if (cl)
				luacon_model->RemoveNotification(this);
			// pop(1)
			lua_settop(ls0, idx);
		}
	};
...
void LuaScriptInterface::initStickmanAPI()
{
...

	{
		const char *flags_str[] {
			...
			"NO_EMIT_PLASMA", "ENABLE_VAC_WALL", "SHOOT_VAC", "SUSPENDED"
		};

		const int g_count = 12, l_count = 2;
		int i;

		lua_createtable(l, 0, g_count);
		for (i = 0; i < g_count; i++)
		{
			lua_pushinteger(l, 1 << i);
			lua_setfield(l, -2, flags_str[i]);
		}
		lua_setfield(l, -2, "g"); // g = "global" property
		
		lua_createtable(l, 0, l_count);
		for (i = g_count; i < g_count + l_count; i++)
		{
			lua_pushinteger(l, 1 << (i - g_count + 3));
			lua_setfield(l, -2, flags_str[i]);
		}
		lua_setfield(l, -2, "l"); // l = "local" property (not Lua keyword)
	}
...
int LuaScriptInterface::stickman_parent(lua_State * l)
{
	...
	playerst* stickman = Element_STKM::_get_playerst(luacon_sim, stickmanID);
	if (stickman == NULL)
		lua_pushinteger(l, -1);
	else
...
int LuaScriptInterface::stickman_newChild(lua_State * l)
{
	if (lua_gettop(l) < 2)
		return luaL_error(l, "Invalid argument length");
	int x = luaL_checkinteger(l, 1);
	int y = luaL_checkinteger(l, 2);
	int p = luaL_optinteger(l, 3, -1);
	playerst * ps = Element_STKM::_get_playerst(luacon_sim, p);
	int np = -1, c = -1;
	if (ps != NULL && ps->spwn)
	{
		np = luacon_sim->create_part(-3, x, y, PT_FIGH);
		if (np >= 0)
		{
			c = luacon_sim->parts[np].tmp;
			// playerst * cs = Element_STKM::_get_playerst(luacon_sim, c);
			Element_STKM::createSTKMChild(luacon_sim, ps, p, c);
		}
	}
	lua_pushinteger(l, np);
	lua_pushinteger(l, c);
...
int LuaScriptInterface::stickman_flags(lua_State * l)
{
	...
	playerst* st = Element_STKM::_get_playerst(luacon_sim, stkid);
	int* prop = NULL;
	if (st != NULL && st->spwn)
		prop = &st->__flags;
	else if (stkid == _STKM_GLOBAL_P)
		prop = &luacon_sim->Extra_FIGH_pause;
	else if (stkid == _STKM_GLOBAL_P + 1)
		prop = &Element_STKM::lifeinc[4];
...
bool LuaScriptInterface::simulation_dll_set_loaded(bool b)
{
	...
	else if (b)
	{
		...
		// <<<< DLL config start >>>>
		const char *dlllibname	= "tptplugin.dll";
		...
		size_t strlen_dllcstr	= strlen(dllcstr);
...

		{
			...
			dll_alloc_str = (char*)malloc(strlen_dllcstr + load_dll_sz + 2);
			strcpy(dll_alloc_str, dllcstr);
			dll_alloc_str[strlen_dllcstr] = '_';
			strlen_dllcstr++;
		}
		HMODULE DLLFuncPack;
		if (dll_alloc_str != NULL)
		{
			DLLFuncPack = LoadLibrary(dlllibname);
			rb = (DLLFuncPack != NULL);
		}
		if (rb)
		{
			...
			{
				dll_alloc_sfx[0] = hexdigits[i>>4];
				dll_alloc_sfx[1] = hexdigits[i&15];
				dll_alloc_sfx[2] = '\0';
			...
			strcpy(dll_alloc_sfx, _loadstr);
			{
				struct _MY_SEH_ST {
					...
					{
						void *_ctx = __builtin_alloca(sizeof(MYCTX)), *octx = this->ctx;
						this->ctx = _ctx;
						LuaScriptInterface::_dll_eh_proc0(args);
						this->ctx = octx;
					}
					static _MY_SEH_ST * eh_fram_typ0 (void * ef)
					{
						return (_MY_SEH_ST *)(ef - offsetof(_MY_SEH_ST, next_eh));
					}
					static bool in_stack_and_aligned (uintptr_t p)
					{
					...
					static int eh_func0 (EXCEPTION_RECORD * er, void * ef, CONTEXT * cr, void * dc)
					{
						...
						jmp_buf _rec;
						_MY_SEH_ST *frame_st = eh_fram_typ0(ef), _nest_seh;
						MYCTX * _ctx = (MYCTX *)frame_st->ctx;

						if (_ctx == NULL || ((intptr_t)_ctx & 3) || (
							!in_stack_and_aligned((uintptr_t)frame_st->next_eh) &&
				...
				_my_seh = (_MY_SEH_ST *)__builtin_alloca(sizeof(_MY_SEH_ST));
				...
				{
					_my_seh->eh_enter((FARPROC)(_MY_SEH_ST::eh_func0));
					if (dllfn != NULL)
					{
						struct {
							FARPROC res1;
...
int LuaScriptInterface::simulation_dll_index(lua_State * l)
{
#ifdef TPT_NEED_DLL_PLUGIN
	struct _funcs {
		static int _sym (lua_State * L)
		{
			FARPROC ret_value = NULL;
			if (luacon_ci->simulation_dll_st.loaded)
			{
				size_t proc_len;
				int proc_i = lua_tointeger(L, 1);
				const char *proc_name = lua_tolstring(L, 2, &proc_len);
				// lua_upvalueindex
				if (proc_name)
				{
					bool _changing = (proc_i >= 0 && proc_i < MAX_DLL_FUNCTIONS);
					if (proc_len && _changing)
					{
						ret_value = GetProcAddress((HMODULE)(luacon_ci->simulation_dll_st.module), proc_name);
						_changing = (ret_value != NULL);
					}
					if (_changing)
						simulation_debug_trigger::dll_check_write(proc_i, ret_value);
				}
				else
					ret_value = (FARPROC)simulation_debug_trigger::dll_check_ex(proc_i, lua_toboolean(L, 2));
			}
			if (ret_value != NULL)
				lua_pushlightuserdata(L, (void*)ret_value);
			else
				lua_pushnil(L);
			return 1;
		}
	...
	case 'l':
#ifdef TPT_NEED_DLL_PLUGIN
		if (!strcmp(keyl+1, "sym"))
		{
			// lua_push
			return lua_pushcclosure(l, _funcs::_sym, 0), 1;
		}
#else
		if (false) {}
#endif
...
int LuaScriptInterface::simulation_dll_newindex(lua_State * l)
{
	...
	case 'e':
		if (!strcmp(keyl+1, "rrfunc"))
		{
			luacon_ci->simulation_dll_st.ehandler = lua_tointeger(l, 3);
		}
		return 0;
	case 'l':
		if (keyl[1] != 'o')
...
//			*((String*)propertyAddress) = ByteString(luaL_checkstring(l, stackPos)).FromUtf8();
			{
				...
				if (_state == 1)
				{
					s2 = strlist_add(&luacon_elem_strlist, s1);
					if (s2 == NULL)
						_state = 2;
				}
				if (_state == 2)
...
int LuaScriptInterface::elements_loadDefault(lua_State * l)
{
	...
	else
	{
		...
		EnterCriticalSection(cs0);
#endif
		strlist_free(&luacon_elem_strlist, true);
...
int LuaScriptInterface::elements_allocate(lua_State * l)
{
	...
	for(int i = PT_NUM-1; i >= 0; i--)
	{
		if(!luacon_sim->elements[i].Enabled)
		{
			...
			luacon_sim->elements[i].Identifier = _identif;
			break;
		}
	}
