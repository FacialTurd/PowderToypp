...
int Element_SLCN::update(UPDATE_FUNC_ARGS)
{
	...
	if (phase & 0x1000)
	{
		...
		parts[i].tmp = (parts[i].tmp & 0xFFF00000) | (phase & 0xFFF) | ((rand() % 16) << 16) | ((parts[i].tmp >> 4) & 0xF000);
	}
	else
	{
		// update phase
...
//#TPT-Directive ElementHeader Element_SLCN static void initSparkles(Particle &part)
void Element_SLCN::initSparkles (Particle &part)
{
	int tmp = (rand() + (rand() << 15)) % 0x900000;
	part.tmp = tmp + (tmp < 0 ? 0xA00000 : 0x100000);
}
